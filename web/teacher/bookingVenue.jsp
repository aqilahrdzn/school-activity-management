<%-- 
    Document   : bookingVenue
    Created on : May 19, 2025, 2:33:04 PM
    Author     : Lenovo
--%>
                ...
                <%
                    ...
                    if (startParam != null && durationParam != null && eventIdParam != null) {
                        try {
                            ...
                            formattedStart = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(startTime);

                            ClassroomDAO dao = new ClassroomDAO();
                            available = dao.getAvailableDuring(startTime, endTime);
                        } catch (Exception e) {
                %>
                <p style="color:red;">Invalid input or system error: <%= e.getMessage()%></p>
                <%
                        }
                    }
                    ...
                    if (eventIdParam != null) {
                        for (Event e : myEvents) {
                            if (String.valueOf(e.getId()).equals(eventIdParam)) {
                                preselectedEvent = e;
                                break;
                            }
                        }
                                    ...
                                    <div class="card-body">
                                        ...
                                        <form class="forms-sample" action="bookingClass.jsp" method="get">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <label for="eventId">Select Your Event:</label>
                                                <% if (eventIdParam != null && startParam != null && endParam != null) {%>
                                                <div class="form-group">
                                                    <label>Latest Created Event:</label>
                                                    <p><strong>Title:</strong> <%= eventIdParam%></p>
                                                    <p><strong>Start Time:</strong> <%= startParam%></p>
                                                    <p><strong>End Time:</strong> <%= endParam%></p>

                                                    <!-- Hidden inputs to send values in the request -->
                                                    <input type="hidden" name="eventTitle" value="<%= eventIdParam%>" />
                                                    <input type="hidden" name="eventStartTime" value="<%= startParam%>" />
                                                    <input type="hidden" name="eventEndTime" value="<%= endParam%>" />
                                                </div>
                                                <% } else { %>
                                            ...
                                            <%
                                                // enable button when all three values exist in THIS request
                                                String pTitle = eventIdParam;
                                                String pStart = startParam;
                                                String pEnd = endParam;

//                                                out.println(pTitle);
//                                                out.println(pStart);
//                                                out.println(pEnd);
                                                boolean buttonEnabled
                                                        = pTitle != null && !pTitle.isBlank()
                                                        && pStart != null && !pStart.isBlank()
                                                        && pEnd != null && !pEnd.isBlank();
